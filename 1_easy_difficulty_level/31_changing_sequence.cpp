...
Using the C++ language, have the function ChangingSequence(arr) take the array of numbers stored in arr and return the index at which 
the numbers stop increasing and begin decreasing or stop decreasing and begin increasing. For example: if arr is [1, 2, 4, 6, 4, 3, 1] 
then your program should return 3 because 6 is the last point in the array where the numbers were increasing and the next number begins a decreasing sequence. 
...
int ChangingSequence(vector<int> numbers) { 
  ...
  for (; si != numbers.size(); si++) {
  	...
  	if (numbers[0] != numbers[si]) break;
  
  }

  bool ascending_order { numbers[0] < numbers[si] ? true : false };

  for (size_t i{si}; i != numbers.size(); i++) {
  	
  	if (ascending_order && (numbers[i-1] > numbers[i])) return (i - 1);
