...
Using the C++ language, have the function VowelSquare(strArr) take the strArr parameter being passed which will be a 2D matrix of some arbitrary size 
...
string VowelSquare(string* str_arr, const size_t row_size) { 	
	...
	for (size_t i{}; i < row_size; i++) {	
		...
		for (auto& ch : str_arr[i]) ch = static_cast<char>(tolower(ch));
	}
	...
	for (size_t i{}; i < (row_size - 1); i++) {
		...
		for (size_t j{}; j != (prev_line.length() - 1); j++) {
			...
			if ( (vowels.find(prev_line[j]) != end(vowels)) && (vowels.find(prev_line[j+1]) != end(vowels))
				&& (vowels.find(next_line[j]) != end(vowels)) && (vowels.find(next_line[j+1]) != end(vowels)) ) {
