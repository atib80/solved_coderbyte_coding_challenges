...
Using the C++ language, have the function LargestFour(arr) take the array of integers stored in arr, and find the four largest elements and return their sum. 
...
int LargestFour(int* arr, const size_t arr_size) { 
  ...
  sort(arr, arr + arr_size, [](const int l, const int r) {
    return (l > r);
  });

  const size_t last_index { arr_size >= 4 ? 4 : arr_size };

  for (size_t i{}; i != last_index; i++) sum += arr[i];
