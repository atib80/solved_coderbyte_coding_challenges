...
Using the C++ language, have the function MultiplicativePersistence(num) take the num parameter being passed which will always be a positive integer 
...
int MultiplicativePersistence(int num) {
  ...
  while (num > 9) {
  	...
  	for (const char digit : num_str) num *= static_cast<int>(digit - '0');
