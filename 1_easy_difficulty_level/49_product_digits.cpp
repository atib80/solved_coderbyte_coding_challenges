...
Using the C++ language, have the function ProductDigits(num) take the num parameter being passed which will be a positive integer, 
...
size_t ProductDigits(const size_t num) { 
  ...
  size_t current_min_len{string::npos};

  for(size_t i{1u}; i < (static_cast<size_t>(sqrt(num)) + 1u); i++)
  {
      ...
      if (num == i * factor)
      {
          ...
          if(result.length() < current_min_len) current_min_len = result.length();          
      }
  }
