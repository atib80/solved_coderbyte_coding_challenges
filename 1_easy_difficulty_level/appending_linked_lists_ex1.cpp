...
struct node {
	...
	~node() {
		if (next) {
			cout << "Deleting next node ("<< next->data << ") in singly-linked list...\n";
			delete next;
		}
	}
};
...
int main() {
	...
	for (auto current_node = list_front_head; current_node; current_node = current_node->next) {
		cout << current_node->data << ' ';
	}

    cout << "\n\nDeleting head node (" << list_front_head->data << ") of singly-linked list...\n";
