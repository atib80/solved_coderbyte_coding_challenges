...
Using the C++ language, have the function MaximalRectangle(strArr) take the strArr parameter being passed
which will be a 2D matrix of 0 and 1's, and determine the area of the largest rectangular submatrix
that contains all 1's. For example: if strArr is ["10100", "10111", "11111", "10010"]
...
bool find_maximal_rectangle_in_matrix(const vector<vector<int>> &matrix, const size_t width,
                                      const size_t height)
{
...
bool check_if_matrix_has_one_elements(const vector<vector<int>> &matrix)
{
    ...
    {
        if (any_of(begin(row), end(row), [](const int value) { return (1 == value); }))
            return true;
    }
...
{
    const auto matrix = create_matrix_from_input_string_array(str_arr, str_arr_size);
