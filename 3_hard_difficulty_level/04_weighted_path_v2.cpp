...
void find_least_costly_route(const unordered_map<string, unordered_map<string, size_t>>&
                             adjacent_vertices_and_their_weights,
                             ...
                             const size_t path_weight = 0)
{
  ...
  {
    if (already_visited_vertices.find(neighbor_vertex.first) != end(already_visited_vertices)
...
string weighted_path_v2(string* str_arr, const size_t str_arr_size)
{
  ...
  create_graph_adjacency_list(str_arr, str_arr_size, vertices, adjacent_vertices_and_their_weights);
