...
string weighted_path_v1(string* str_arr, const size_t str_arr_size) {
  vector<string> vertices{};
  unordered_map<string, unordered_map<string, size_t>>
      adjacent_vertices_and_their_weights{};
  ...
  while (!q.empty()) {
    ...
    const auto neighbor_vertex_iter{
        adjacent_vertices_and_their_weights[list_iter->current_vertex].find(
            vertices.back())};

    if (neighbor_vertex_iter !=
            end(adjacent_vertices_and_their_weights[list_iter
                                                        ->current_vertex]) &&
        list_iter->path_weight + neighbor_vertex_iter->second <
    ...
    for (const auto& neighbor_vertex :
      ...
      if (list_iter->path_weight + neighbor_vertex.second >=
          shortest_path_distance)
        continue;
