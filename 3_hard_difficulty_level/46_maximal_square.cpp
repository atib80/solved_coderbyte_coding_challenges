...
string maximal_square(const string* str_arr, const size_t row_size) {
  const size_t column_size{str_arr[0].length()};

  for (size_t i{1}; i < row_size; i++) {
    if (column_size != str_arr[i].length())
      return "not possible";
  }
    ...
    for (size_t j{}; j + max_square_size <= column_size; j++) {
      for (size_t i{}; i + max_square_size <= row_size; i++) {
        for (size_t k{}; k < max_square_size; k++) {
          ...
          if (count == max_square_size)
            return to_string(max_square_size * max_square_size);
        }
      }
    }
