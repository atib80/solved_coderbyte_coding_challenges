...
Using the C++ language, have the function IntersectingLines(strArr) take strArr
...
vector<pair<int, int>> parse_2d_coordinate_pairs(string* str_arr,
  ...
  for (size_t i{}; i < str_arr_size; i++) {
    ...
    const vector<string> number_parts{split(str_arr[i], ",")};

    if (2 != number_parts.size())
      return vector<pair<int, int>>{};

    coordinates.emplace_back(
        make_pair(stoi(number_parts[0]), stoi(number_parts[1])));
  }
...
pair<int, int> factorize_rational_number(int nominator, int denominator) {
  const bool is_rational_number_negative{(nominator < 0 && denominator > 0) ||
  ...
  for (int i{min_number}; i > 1; i--) {
    if (nominator % i == 0 && denominator % i == 0) {
      ...
      if (is_rational_number_negative)
        nominator = -nominator;
...
string find_intersection_point_coordinates_of_two_lines(
...
string IntersectingLines(string* str_arr, const size_t str_arr_size) {
  ...
  return find_intersection_point_coordinates_of_two_lines(points[0], points[1],
                                                          points[2], points[3]);
}
...
int main() {
  ...
  cout << IntersectingLines(d, sizeof(d) / sizeof(*d))
       << '\n';  // expected output: "nointersection"
