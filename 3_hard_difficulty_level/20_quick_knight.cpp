...
vector<pair<int, int>> find_next_positions(const pair<int, int>& current_pos)
{
	vector<pair<int, int>> next_positions{};

	// can we go N:2-W:1 and/or N:2-E:1?
	if (current_pos.first - 2 >= 1)
	{
		if (current_pos.second - 1 >= 1)
			next_positions.emplace_back(
				make_pair(current_pos.first - 2, current_pos.second - 1));
		if (current_pos.second + 1 <= 8)
			next_positions.emplace_back(
				make_pair(current_pos.first - 2, current_pos.second + 1));
	}

	// can we go S-2-W:1 and/or S:2-E:1?
	if (current_pos.first + 2 <= 8)
	{
		if (current_pos.second - 1 >= 1)
			next_positions.emplace_back(
				make_pair(current_pos.first + 2, current_pos.second - 1));
	...
	// can we go W:2-N:1 and/or E:2-N:1?
	if (current_pos.first - 1 >= 1)
	{
		if (current_pos.second - 2 >= 1)
			next_positions.emplace_back(
				make_pair(current_pos.first - 1, current_pos.second - 2));
		if (current_pos.second + 2 <= 8)
			next_positions.emplace_back(
				make_pair(current_pos.first - 1, current_pos.second + 2));
	}

	// can we go W-2-S:1 and/or E:2-S:1?
	if (current_pos.first + 1 <= 8)
	{
		if (current_pos.second - 2 >= 1)
			next_positions.emplace_back(
				make_pair(current_pos.first + 1, current_pos.second - 2));
...
void find_minimum_number_of_jumps(const vector<vector<int>>& chessboard, const pair<int, int>& current_pos,
                                  const pair<int, int>& target_pos,
                                  size_t& min_number_of_jumps, const size_t current_number_of_jumps = 0)
{
	if ((current_pos == target_pos) && (current_number_of_jumps < min_number_of_jumps))
	{
		min_number_of_jumps = current_number_of_jumps;
		return;
	}

	if (current_number_of_jumps >= min_number_of_jumps || current_number_of_jumps >= 32) return;

	const vector<pair<int, int>> next_positions{find_next_positions(current_pos)};
	...
	for (const auto& next_pos : next_positions)
	{
		...
		find_minimum_number_of_jumps(chessboard_visited, next_pos, target_pos, min_number_of_jumps,
...
string QuickKnight(string str)
{
	...
	chessboard[knight_pos.first - 1][knight_pos.second - 1];
	size_t min_number_of_jumps{string::npos};
	find_minimum_number_of_jumps(chessboard, knight_pos, target_pos, min_number_of_jumps, 0);
