...
Using the C++ language, have the function CityTraffic(strArr) read strArr which will be a representation of an undirected graph
in a form similar to an adjacency list. Each element in the input will contain an integer which will represent the population for that city,
and then that will be followed by a comma separated list of its neighboring cities and their populations (these will be separated by a colon).
...
For example: if every single person in all the cities decided to go to city 7, then via the upper road the number of people 
coming in would be (8 + 38) = 46. If all the cities beneath city 7 decided to go to it via the lower road, the number of people 
coming in would be (2 + 15 + 1 + 3 + 4 + 5) = 30. 
...
city:max_traffic,city:max_traffic,... The cities should be outputted in sorted order by the city number. 
...
struct node
{
	...
	void add_neighbor_node(const int neighbor_id)
	{
		if (find(begin(neighbors), end(neighbors), neighbor_id) == end(neighbors))
...
int find_max_traffic_for_graph_node(const map<int, node>& graph, const int node_id, unordered_set<int>& visited_nodes)
{
	if (graph.find(node_id) == end(graph)) return 0;

	int max_traffic{node_id};

	const auto current_node_neighbors = graph.find(node_id)->second.neighbors;

	for (const int next_neighbor_node_id : current_node_neighbors)
	{
		if (visited_nodes.find(next_neighbor_node_id) != end(visited_nodes)) continue;

		visited_nodes.insert(next_neighbor_node_id);

		max_traffic += find_max_traffic_for_graph_node(graph, next_neighbor_node_id, visited_nodes);
...
map<int, node> create_city_graph_from_input_string_array(string* str_arr, const size_t str_arr_size)
{
...
string find_maximum_traffic_for_all_city_nodes(const map<int, node>& city_graph)
{
	ostringstream oss{};
	...
	for (const auto& current_node : city_graph)
	{
		...
		for (const int neighbor_node_id : current_node.second.neighbors)
		{
			...
			const int current_neighbor_node_max_traffic{
				find_max_traffic_for_graph_node(city_graph, neighbor_node_id, visited_nodes)
			};
...
{
	const map<int, node> city_graph{create_city_graph_from_input_string_array(str_arr, str_arr_size)};
...
int main()
{
	...
	string B[] = {"1:[5]", "4:[5]", "3:[5]", "5:[1,4,3,2]", "2:[5,15,7]", "7:[2,8]", "8:[7,38]", "15:[2]", "38:[8]"};
	...
	string D[] = {"1:[5]", "2:[5,18]", "3:[5,12]", "4:[5]", "5:[1,2,3,4]", "18:[2]", "12:[3]"};
