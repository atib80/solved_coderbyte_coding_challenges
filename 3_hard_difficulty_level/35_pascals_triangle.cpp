...
Using the C++ language, have the function PascalsTriangle(arr) take arr which will be a partial row from Pascal's triangle.
Pascal's triangle starts with a [1] at the first row of the triangle.
Then the second row is [1,1] and the third row is [1,2,1].
The next row begins with 1 and ends with 1, and the inside of the row is determined
by adding the k-1 and kth elements from the previous row.
The next row in the triangle would then be [1,3,3,1], and so on.
The input, arr will be some almost completed row from the triangle, for example [1,4,6,4]
and your program should return the next element in that row.
...
The input array will contain at least 2 elements and array inputs such as [1] and [1,1] will not occur as inputs.
...
const vector<int> &generate_next_pascal_triangle_row(
    const pair<bool, int> &reset_state_to_specific_row = pair<bool, int>{false, 0})
{
...
const vector<vector<int>> &generate_pascal_triangle_rows(const size_t pascal_row_number)
{
    ...
    for (size_t i{3}; i <= pascal_row_number; i++)
    {
        ...
        for (size_t j{}; j != prev_pascal_triangle_numbers.size() - 1; j++)
            next_pascal_triangle_numbers.emplace_back(prev_pascal_triangle_numbers[j] + prev_pascal_triangle_numbers[j + 1]);
...
int PascalsTriangle(const int *arr, const size_t arr_size)
{
    ...
    {
        const vector<int> &next_pascal_triangle_row{generate_next_pascal_triangle_row()};
