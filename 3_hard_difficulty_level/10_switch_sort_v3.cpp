/*
Coderbyte coding challenge: Switch Sort v3

(iterative solution implemented by using brute-force method)
...
int SwitchSort_v3(const size_t* arr, const size_t arr_size) {
...
int main() {
  ...
  // cout << SwitchSort_v3(A, sizeof(A)/sizeof(*A));
  const size_t B[] = {1, 3, 4, 2};
  cout << SwitchSort_v3(B, sizeof(B) / sizeof(*B))
       << '\n';  // expected output: 2
  const size_t C[] = {3, 1, 2};
  cout << SwitchSort_v3(C, sizeof(C) / sizeof(*C))
       << '\n';  // expected output: 2
  const size_t D[] = {1, 3, 4, 2};
  cout << SwitchSort_v3(D, sizeof(D) / sizeof(*D))
       << '\n';  // expected output: 2
  const size_t E[] = {3, 4, 2, 1};
  cout << SwitchSort_v3(E, sizeof(E) / sizeof(*E))
       << '\n';  // expected output: 3
  const size_t F[] = {4, 3, 2, 1};
  cout << SwitchSort_v3(F, sizeof(F) / sizeof(*F))
       << '\n';  // expected output: 2
  const size_t G[] = {3, 4, 2, 1};
  cout << SwitchSort_v3(G, sizeof(G) / sizeof(*G))
       << '\n';  // expected output: 3
  const size_t H[] = {4, 3, 2, 1};
  cout << SwitchSort_v3(H, sizeof(H) / sizeof(*H))
       << '\n';  // expected output: 2
  const size_t I[] = {5, 4, 3, 2, 1};
  cout << SwitchSort_v3(I, sizeof(I) / sizeof(*I))
       << '\n';  // expected output: 2
  const size_t J[] = {5, 3, 4, 1, 2};
  cout << SwitchSort_v3(J, sizeof(J) / sizeof(*J))
       << '\n';  // expected output: 4
  const size_t K[] = {5, 4, 3, 1, 2};
  cout << SwitchSort_v3(K, sizeof(K) / sizeof(*K))
       << '\n';  // expected output: 3
