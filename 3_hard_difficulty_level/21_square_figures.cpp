...
int64_t find_smallest_number_for_specified_length(
  ...
  const vector<int64_t> decrement_factors{0LL,
                                          1LL,
                                          10LL,
                                          100LL,
                                          1'000LL,
                                          ...
                                          10'000'000'000LL,
                                          100'000'000'000LL};
  const size_t initial_decrement_factor_index{
      to_string(current_number).length()};

  // could decrement number by a certain offset value (100'000'000'000LL,
  // 10'000'000'000LL, ... 1'000, 100, 10 and then finally by 1 (if required)
  // depending on the original initial length's value (specified by
  // 'target_number_len')
...
int64_t SquareFigures(const size_t target_number_len) {
  ...
  int64_t lower_bound{},
      upper_bound{static_cast<int64_t>(sqrt(numeric_limits<int64_t>::max()))};
