...
which represents a heap data structure and determine whether or not it is a max heap.
A max heap has the property that all nodes in the heap are either greater than or equal
...
void find_incorrect_max_heap_values(const int* arr, const size_t arr_size, const size_t pos, ostringstream& oss)
{
	if (pos >= arr_size) return;

	if (arr[pos] > arr[pos / 2]) oss << arr[pos] << ',';

	if ((pos + 1 < arr_size) && (arr[pos + 1] > arr[pos / 2])) oss << arr[pos + 1] << ',';

	if (2 * pos + 1 < arr_size) find_incorrect_max_heap_values(arr, arr_size, 2 * pos + 1, oss);

	if (2 * (pos + 1) + 1 < arr_size) find_incorrect_max_heap_values(arr, arr_size, 2 * (pos + 1) + 1, oss);
}

string MaxHeapChecker(const int* arr, const size_t arr_size)
{
	if (1u == arr_size) return "max";

	ostringstream oss{};

	find_incorrect_max_heap_values(arr, arr_size, 1, oss);
