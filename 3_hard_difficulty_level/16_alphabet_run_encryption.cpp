...
string insert_delimiters_into_string_at_appropriate_places(
    const string& encoded_str) {
  string padded_encoded_str{encoded_str};
  size_t start{encoded_str.length() - 1};

  while ((start = padded_encoded_str.find_last_of("LRSN", start)) !=
         string::npos) {
    switch (padded_encoded_str[start]) {
      case 'N':
        if (start < padded_encoded_str.length() - 1 &&
            '|' != padded_encoded_str[start + 1])
...
string AlphabetRunEncryption(string str) {
  str = trim(str);
  const string encoded_str{
      insert_delimiters_into_string_at_appropriate_places(str)};
  ...
  for (const string& seq : str_parts) {
    size_t start{};
    ...
    if ((start = seq.find_first_of("LRSN", start)) != string::npos) {
      switch (seq[start]) {
        ...
        case 'S':
          if (seq[start - 2] == previous_simple_seq_char)
            ...
            decoded_str.append({seq[start - 2], seq[start - 1]});
          previous_simple_seq_char = seq[start - 1];
          break;
        ...
        case 'L':
          if (get_next_char(seq[start - 1]) == previous_simple_seq_char) {
            ...
            decoded_str.append(
                {get_next_char(seq[start - 1]), get_prev_char(seq[start - 1])});
          }
        ...
        case 'R':
          if (get_prev_char(seq[start - 1]) == previous_simple_seq_char) {
            ...
            decoded_str.append(
                {get_prev_char(seq[start - 1]), get_next_char(seq[start - 1])});
          }
      ...
      while (index < seq_len - 1) {
        ...
        for (size_t i{index + 1}; i < seq_len; i++) {
          if (seq[i] == seq[i - 1] || abs(seq[i] - seq[i - 1]) > 1 ||
              seq_len - 1 == i) {
            if (!is_prev_decoded_char_seq) {
              ...
              if (is_dir_forward) {
                if (get_prev_char(fc) == previous_simple_seq_char) {
                  ...
                  decoded_str.append(
                      {get_prev_char(fc), get_next_char(seq[i])});
                  previous_simple_seq_char = get_next_char(seq[i]);
                }
              } else {
                if (get_next_char(fc) == previous_simple_seq_char) {
                  ...
                  decoded_str.append(
                      {get_next_char(fc), get_prev_char(seq[i])});
                  previous_simple_seq_char = get_prev_char(seq[i]);
                }
              }
            } else {
...
int main() {
  ...
  cout << AlphabetRunEncryption(
              move(string{"cdefghijklmnopqrstuvwxxwvusrqponmlkjihgf"}))
       << '\n';
