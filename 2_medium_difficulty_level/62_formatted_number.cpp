...
Using the C++ language, have the function FormattedNumber(strArr) take the strArr parameter being passed, which will only contain a single element, 
...
For example: if strArr is ["1,093,222.04"] then your program should return the string true, but if the input were ["1,093,22.04"] then your program 
should return the string false. The input may contain characters other than digits.
...
string FormattedNumber(string* str_arr) {
  ...
  *str_arr = trim(*str_arr);
  ...
  if ((string::npos != dot_pos) && ((dot_pos == str_arr->length() - 1) || (str_arr->find_first_of("0123456789", dot_pos + 1) != dot_pos + 1))) return "false";

  const size_t scientific_sign_pos { str_arr->find_first_of("eE") };

  const size_t sign_pos { str_arr->find_first_of("+-") };

  if ((string::npos == scientific_sign_pos) && (string::npos != sign_pos) && (0u != sign_pos)) {

  	return "false";

  } else {

  	if ( (scientific_sign_pos == (str_arr->length() - 1)) || ( (str_arr->find_first_of("+-0123456789", scientific_sign_pos + 1) != scientific_sign_pos + 1) &&
  		((str_arr->length() > scientific_sign_pos + 2) && (str_arr->find_first_of("+-0123456789", scientific_sign_pos + 2) != scientific_sign_pos + 2) ) ) )  return "false";

  	if ( (str_arr->find_first_of("+-", scientific_sign_pos + 1) == scientific_sign_pos + 1) && 
  	     ((str_arr->length() > scientific_sign_pos + 2) && (str_arr[0][scientific_sign_pos + 2] < '0' || str_arr[0][scientific_sign_pos + 2] > '9'))) return "false";
  }

  if (string::npos != str_arr->find_first_not_of("1234567890,.+-eE")) return "false";

  const auto number_of_commas = count(begin(*str_arr), end(*str_arr), ',');

  const size_t whole_number_part_len { string::npos != dot_pos ? dot_pos : ( scientific_sign_pos != string::npos ? scientific_sign_pos : str_arr->length() ) };
  
  if (number_of_commas != 0) {	  	

	  	const string whole_number_part { str_arr->substr(0, whole_number_part_len) };

	  	vector<string> number_parts = split(whole_number_part, ",");

	  	for (size_t i{number_parts.size() - 1}; i != 0; i--) {

	  		if (3 != number_parts[i].length()) return "false";

	  	}

  } else if (whole_number_part_len > 3u) return "false";  
