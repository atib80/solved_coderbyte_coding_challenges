...
Using the C++ language, have the function TrappingWater(arr) take the array of
...
will be trapped between the two tallest buildings. For example: if arr is [3, 0,
0, 2, 0, 4] then this array of building heights looks like the following picture
if we draw it out:
               __
 __           |  |
|  |     __   |  |
...
look like the following (the x's represent water): This is the most water that
can be trapped in this picture, and if you calculate the area you get 10, so
your program should return 10.
...
size_t TrappingWater(const int* arr, const size_t arr_size) {
  ...
  while (!arr[start] && start <= last)
    start++;  // ignore water levels with 0 height at the beginning of arr
  while (!arr[last] && last > start)
    last--;  // ignore water levels with 0 height at the end of arr

  if (last - start < 3 ||
      all_of(arr + start, arr + last + 1,
             [&](const int water_level) { return water_level == arr[start]; }))
    return 0;

  vector<int> water_levels(arr + start, arr + last + 1);

  int first_max_water_level{water_levels.front()};

  int last_max_water_level{water_levels.back()};

  vector<int> current_water_levels{};

  for (size_t i{1}; i < water_levels.size(); i++) {
    ...
    if (water_levels.size() - 1 == i &&
      ...
      for (const int water_level : current_water_levels) {
        if (water_level >= first_max_water_level)
          continue;
        count += first_max_water_level - water_level;
      }
