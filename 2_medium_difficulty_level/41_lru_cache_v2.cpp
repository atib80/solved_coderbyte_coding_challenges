...
string get_lru_cache_contents(const deque<char>& lru_cache) {
  return join(lru_cache, "-");
}

deque<char> build_lru_cache_contents(const string* str_arr,
                                     const size_t str_arr_size) {
  deque<char> lru_cache{};
  unordered_map<char, size_t> lru_cache_index{};
  ...
  for (const string* current_item{str_arr};
    ...
    if (lru_cache_index.find(cache_item) != end(lru_cache_index)) {
      lru_cache.erase(begin(lru_cache) + lru_cache_index[cache_item]);
...
string LRUCache(const string* str_arr, const size_t str_arr_size) {
  const deque<char> lru_cache{build_lru_cache_contents(str_arr, str_arr_size)};
