...
Using the C++ language, have the function MaxSubarray(arr) take the array of
numbers stored in arr and determine the largest sum that can be formed by any
contiguous subarray in the array. For example, if arr is [-2, 5, -1, 7, -3] then
your program should return 11 because the sum is formed by the subarray [5, -1,
7]. Adding any element before or after this subarray would make the sum smaller.
...
int MaxSubarray(const int* arr, const size_t arr_size) {
  int current_largest_sum{*max_element(arr, arr + arr_size)};
  size_t current_sub_array_size{arr_size};

  if (arr + arr_size ==
      find_if(arr, arr + arr_size, [](const int n) { return n < 0; }))
    return accumulate(arr, arr + arr_size, 0);
...
int main() {
  ...
  const int E[] = {1, 2, 3, 4, 5};
  cout << MaxSubarray(E, sizeof(E) / sizeof(*E)) << '\n';  // 15
