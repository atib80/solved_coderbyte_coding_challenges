...
Using the C++ language, have the function StringExpression(str) read the str
parameter being passed which will contain the written out version of the numbers
0-9 and the words "minus" or "plus" and convert the expression into an actual
final number written out as well. For example: if str is
"foursixminustwotwoplusonezero" then this converts to "46 - 22 + 10" which
evaluates to 34 and your program should return the final string threefour. If
your final answer is negative it should include the word "negative."
...
string StringExpression(string str) {
  ...
  size_t operation_word_offset{
      min(operation_word_plus_offset, operation_word_minus_offset)};

  if (string::npos == operation_word_offset)
    return "not possible";

  size_t operation_word_len =
      operation_word_plus_offset < operation_word_minus_offset ? 4 : 5;
  bool is_add_operation{operation_word_plus_offset <
                        operation_word_minus_offset};
  ...
  while (true) {
    ...
    operation_word_len =
        operation_word_plus_offset < operation_word_minus_offset ? 4 : 5;
    is_add_operation = operation_word_plus_offset < operation_word_minus_offset;
    operations.push_back(is_add_operation);
  ...
  for (size_t i{}; i < numbers_str.size(); i++)
    number_values.emplace_back(
        convert_digit_words_into_decimal_number(numbers_str[i]));
