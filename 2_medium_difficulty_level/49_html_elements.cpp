...
Using the C++ language, have the function HTMLElements(str) read the str parameter being passed which will be a string of HTML DOM elements and plain text. 
The elements that will be used are: b, i, em, div, p. 
For example: if str is "<div><b><p>hello world</p></b></div>" then this string of DOM elements is nested correctly 
...
string check_if_html_document_is_correctly_structured(const string& html_text)
{
	...
	while ((first_start_tag_pos < html_text_len) && ((first_start_tag_pos = html_text.find('<', first_start_tag_pos)) != string::npos))
	{
		if (html_text[first_start_tag_pos + 1] != '/')
		{
			...
			if (string::npos == tag_end_pos) return string{"false"};									

			const string start_tag_name { html_text.substr(first_start_tag_pos + 1, tag_end_pos - (first_start_tag_pos + 1)) };
		...
		else
		{
			...
			if (tag_end_pos == string::npos) return string{"false"};

			const string end_tag_name { html_text.substr(first_start_tag_pos + 2, tag_end_pos - (first_start_tag_pos + 2)) };

			if (!start_tag_names.empty()) {

				if (end_tag_name != start_tag_names.top()) return start_tag_names.top();
	...
	if (!start_tag_names.empty()) return start_tag_names.top();	
...
int main() { 
  ...
  cout << HTMLElements(move(string{"<div>abc</div><p><em><i>test test test</b></em></p>"})) << '\n'; // expected output: "i"
