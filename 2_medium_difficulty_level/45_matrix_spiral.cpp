...
Using the C++ language, have the function MatrixSpiral(strArr) read the array of strings stored in strArr which will represent a 2D N matrix, 
...
string MatrixSpiral(string* str_arr, const size_t str_arr_size) { 
	...
	if (!str_arr_size || !str_arr) return string{};
	...
	if (1u == str_arr_size) {
		...
		*str_arr = trim(*str_arr);
		...
		for (size_t i{}; i < elements_str.size() - 1; i++) {
			elements_str[i] = trim(elements_str[i]);
			oss << elements_str[i] << ',';
		}

		elements_str[elements_str.size() - 1] = trim(elements_str[elements_str.size() - 1]);
		oss << elements_str[elements_str.size() - 1];
		return oss.str();
	}

  vector<vector<string>> matrix(str_arr_size, vector<string>{});
  ...
  for (size_t i{}; i < str_arr_size; i++) {
    ...
    for (size_t j{}; j < elements_str.size(); j++) { 
      ...
      elements_str[j] = trim(elements_str[j]);
		  matrix[i][j] = elements_str[j];
  ...
  while (true) {
  	...
  	if (cells_visited >= max_cells) break;
