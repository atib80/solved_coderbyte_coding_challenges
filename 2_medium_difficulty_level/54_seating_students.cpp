...
Using the C++ language, have the function SeatingStudents(arr) read the array of integers stored in arr which will be in the following format: 
[K, r1, r2, r3, ...] where K represents the number of desks in a classroom, and the rest of the integers in the array will be in sorted order 
and will represent the desks that are already occupied. All of the desks will be arranged in 2 columns, where desk #1 is at the top left, desk #2 is at the top right, 
...
size_t SeatingStudents(const size_t* arr, const size_t arr_size) {
	...
	if (1u == arr_size) return (arr[0] / 2 * arr[0] / 2);	

    const size_t K{arr[0]};

	vector<size_t> seats(K);
		
	for (size_t i{1u}; i != arr_size; i++) seats[arr[i]-1] = 1;
	...
	for (size_t i{}; i != seats.size(); i++) {
		...
		if (!seats[i]) {
			...
			if (i % 2 == 0) {
				...
				if (((i + 1) < K) && !seats[i + 1]) count++;
				if (((i + 2) < K) && !seats[i + 2]) count++;
				
			} else if (((i + 2) < K) && !seats[i + 2]) count++;
