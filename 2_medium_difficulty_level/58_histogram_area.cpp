...
Using the C++ language, have the function HistogramArea(arr) read the array of
...
covered by the x's. The area of that space is equal to 6 because the entire
width is 2 and the maximum height is 3, therefore 2 * 3 = 6. Your program should
return 6. The array will always contain at least 1 element.
...
size_t HistogramArea(const size_t* arr, const size_t arr_size) {
  ...
  for (size_t i{}; i < arr_size; i++) {
    if (already_visited_heights.count(arr[i]))
      continue;
    already_visited_heights.insert(arr[i]);
    size_t current_area{};
